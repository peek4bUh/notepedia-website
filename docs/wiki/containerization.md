...
```shell-session
angel99ab@htb[/htb]$ docker build -t FS_docker .
```
...
```shell-session
angel99ab@htb[/htb]$ docker run -p <host port>:<docker port> -d <docker container name>
```

#### Docker Run

```shell-session
angel99ab@htb[/htb]$ docker run -p 8022:22 -p 8080:80 -d FS_docker
```
...
```shell-session
angel99ab@htb[/htb]$ sudo apt-get install lxc lxc-utils -y
```
...
```shell-session
angel99ab@htb[/htb]$ sudo lxc-create -n linuxcontainer -t ubuntu
```
...
```shell-session
angel99ab@htb[/htb]$ sudo vim /usr/share/lxc/config/linuxcontainer.conf
```
...
```shell-session
angel99ab@htb[/htb]$ sudo systemctl restart lxc.service
```
...
- HTB Academy. (n.d.). _Linux Fundamentals: System Management: Containerization._ [https://academy.hackthebox.com/module/18/section/2097](https://academy.hackthebox.com/module/18/section/2097)
