...
When a system starts up, the kernel initiates a few of its own activities as processes and launches a program called `init`. `init`, in turn, runs a series of shell scripts located in `/etc` called _init scripts_, which start all the system services. Many of these services are implemented as `daemon` programs.
...
Lists two processes, process `342` and process `620`, which are `bash` and `ps`.
...
Adding the `x` option (note that there is no leading dash) tells [[ps(1)]] to show all the processes regardless of what terminal (if any) they are controlled by. The `?` in the TTY column indicates no controlling terminal.
...
To see a more dynamic view of the machine's activity, you can use the [[top]] command.
...
If you run [[ps(1)]], you can see the process.
...
The shell's job control facility also give you a way to list the jobs that have been launched from the terminal. Using the [[jobs]] command, you can see this list:
...
A process in the background is immune from a terminal keyboard input, including any attempt to interrupt it with `CTRL-C`. To return a process to the foreground, use the [[fg]] command:
...
The [[fg]] command followed by a percent sign and the job number called _jobspec_ does the trick. If you have only one background job, the _jobspec_ is optional.
...
You can either continue the program's execution in the foreground, using the [[fg]] command or resume the program's execution in the background with the [[bg]] command.
...
As with the [[fg]] command, the _jobspec_ is optional if there is only one job.
...
The [[kill]] command is used to _kill_ processes. This allows you to terminate programs that need killing (that is, some kind of pausing or termination).
...
The [[kill]] command doesn't kill exactly _kill_ processes, it sends them **signals**. **Signals** are one of several ways that the operating system communicates with programs. `CTRL-C` and `CTRL-Z` are **signals**. When the terminal receives one of these keystrokes, it sends a **signal** to the program in the foreground.
...
|    `15`    |  `TERM`  | Terminate. This is the default signal sent by the [[kill]] command. If a program is still _alive_ enough to receive signals, it will terminate.                                                                                                                                                                                                                                                                                                                                     |
...
Here is an example with the `HUG` signal. You start the `xlogo` program in the background and then send it a `HUP` signal with [[kill]]. The `xlogo` program terminates and the shell indicates that the background process has received a hang-up signal. You may need to press `ENTER` a couple of times before the message appears.
...
It's also posible to send signals to multiple processes matching a specified program or username by using the [[killall]] command.
...
The [[shutdown]] command is a bit more interesting. With it, you can specify which of the actions to perform (halt, power down or reboot) and provide a time delay to the shutdown event. Most often is used like this to halt the system:
...
The delay can be specified in a variety of ways. Once the [[shutdown]] command is executed, a message is _broadcast_ to all logged-in users warning them of the impending event.
...
- Shotts, W. (2019). _The Linux Command Line: Processes_ (2nd ed.). No Starch Press
